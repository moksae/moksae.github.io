...
const App = (() => {
  ...
  function renderProfile() {
    ...
    const postsTotal = vis(S.data.posts.filter(p=>!p.parentId)).length + vis(S.data.reviews).length;
    if (ge('stat-posts'))    ge('stat-posts').textContent    = postsTotal;
    if (ge('stat-writings')) ge('stat-writings').textContent = vis(S.data.writings).length;
    if (ge('stat-pics'))     ge('stat-pics').textContent     = vis(S.data.pics||[]).length;
  }
