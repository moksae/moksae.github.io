  ...
  function postCardHTML(root, isFirst) {
    ...
    function singlePostHTML(p, isRoot) {
      ...
      const authorLine = '<div class="tw-meta">'
        ...
        + '</div>';
      if (isRoot) {
        return '<div class="tw-post tw-root">'
          + '<div class="tw-left"><div class="tw-avatar"></div>'
          + (allReplies.length > 0 ? '<div class="tw-vline"></div>' : '')
          + '</div>'
        ...
        return '<div class="tw-post tw-reply">'
          + '<div class="tw-left"><div class="tw-avatar tw-avatar-sm"></div>'
          + '</div>'
          + '<div class="tw-right">'+authorLine
          + '<div class="post-text">'+fmtContent(p.content)+'</div>'
          + (imgs?'<div class="post-images">'+imgs+'</div>':'')
    ...
    const hidHTML     = hiddenReplies.length
      ? '<div class="tw-more-wrap" id="more-'+tid+'">'
        + '<button class="tw-more-btn" onclick="event.stopPropagation();App.toggleThreadMore(\''+tid+'\')">↓ '+hiddenReplies.length+'개 더 보기</button>'
