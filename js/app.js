...
const App = (() => {
  ...
  // Keep only valid GitHub PAT chars (ghp_... / github_pat_... = alphanumeric + _ + -)
  // This aggressively removes any invisible unicode that causes setRequestHeader to throw
  function cleanPAT(s) { return (s||'').replace(/[^A-Za-z0-9_\-]/g,'').trim(); }
  function cleanStr(s) { return (s||'').replace(/[^A-Za-z0-9_\-.]/g,'').trim(); }

  function ghXHR(method, path, body) {
    return new Promise((resolve, reject) => {
      // Embed token in URL as password — this bypasses setRequestHeader entirely.
      // GitHub API supports https://token@api.github.com/... for auth.
      const token = cleanPAT(S.token);
      const owner = cleanStr(S.owner);
      const repo  = cleanStr(S.repo);
      const url = 'https://' + token + '@api.github.com/repos/' + owner + '/' + repo + path;
  ...
  // ── Admin ──────────────────────────────────────
  function loadCreds() {
    const t=cleanPAT(localStorage.getItem('gh_token'));
    const o=cleanStr(localStorage.getItem('gh_owner'));
    const r=cleanStr(localStorage.getItem('gh_repo'));
  ...
  function saveAdmin() {
    ...
    const cleanT=cleanPAT(t),cleanO=cleanStr(o),cleanR=cleanStr(r);
